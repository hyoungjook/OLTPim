...
class tpcc_sc_worker : public bench_worker, public tpcc_worker_mixin {
 public:
  ...
  static ermia::coro::generator<rc_t> TxnNewOrder(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_new_order(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnDelivery(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_delivery(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnCreditCheck(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_credit_check(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnPayment(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_payment(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnOrderStatus(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_order_status(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnStockLevel(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_stock_level(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnQuery2(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_query2(idx, begin_epoch);
  }
  ...
  static ermia::coro::generator<rc_t> TxnMicroBenchRandom(bench_worker *w, uint32_t idx, ermia::epoch_num begin_epoch) {
    return static_cast<tpcc_sc_worker *>(w)->txn_microbench_random(idx, begin_epoch);
  }
...
ermia::coro::generator<rc_t> tpcc_sc_worker::txn_delivery(uint32_t idx, ermia::epoch_num begin_epoch) {
  ermia::transaction *txn = db->NewTransaction(ermia::transaction::TXN_FLAG_CSWITCH,
                                               arenas[idx],
                                               &transactions[idx],
                                               idx);
...
ermia::coro::generator<rc_t> tpcc_sc_worker::txn_order_status(uint32_t idx, ermia::epoch_num begin_epoch) {
...
ermia::coro::generator<rc_t> tpcc_sc_worker::txn_stock_level(uint32_t idx, ermia::epoch_num begin_epoch) {
...
ermia::coro::generator<rc_t> tpcc_sc_worker::txn_credit_check(uint32_t idx, ermia::epoch_num begin_epoch) {
  /*
...
ermia::coro::generator<rc_t> tpcc_sc_worker::txn_query2(uint32_t idx, ermia::epoch_num begin_epoch) {
...
ermia::coro::generator<rc_t> tpcc_sc_worker::txn_microbench_random(uint32_t idx, ermia::epoch_num begin_epoch) {
  ermia::transaction *txn = db->NewTransaction(ermia::transaction::TXN_FLAG_CSWITCH,
                                               arenas[idx],
                                               &transactions[idx],
                                               idx);
...
bench_worker::workload_desc_vec tpcc_sc_worker::get_workload() const {
  workload_desc_vec w;
...
void tpcc_sc_worker::MyWork(char *) {
...
void tpcc_cs_do_test(ermia::Engine *db) {
  ermia::config::read_txn_type = "tpcc-simple-coro";
  tpcc_parse_options();
  tpcc_bench_runner<tpcc_sc_worker> r(db);
  r.run();
}

int main(int argc, char **argv) {
  bench_main(argc, argv, tpcc_cs_do_test);
  return 0;
}
