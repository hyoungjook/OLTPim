...
#include "../dbcore/histogram.h"

extern void ycsb_do_test(ermia::Engine *db);
extern void ycsb_cs_do_test(ermia::Engine *db);
extern void ycsb_cs_nested_do_test(ermia::Engine *db);
extern void tpcc_do_test(ermia::Engine *db);
extern void tpcc_cs_do_test(ermia::Engine *db);
...
extern volatile uint64_t committed_txn_count;
...
class bench_worker : public ermia::thread::Runner {
 public:
  ...
  typedef coroutine_handle<ermia::coro::generator<rc_t>::promise_type> CoroTxnHandle;
  typedef ermia::coro::generator<rc_t> (*coro_txn_fn_t)(bench_worker *, uint32_t, ermia::epoch_num);
  typedef ermia::coro::task<rc_t> (*task_fn_t)(bench_worker *, ermia::transaction *, uint32_t);
 ...
 private:
  ...
  uint64_t fetch_cold_tx_interval;
